-- This 1st query request all info and save it to make a concrete graph (percentaje of news according to the selected topics)
-- The 2nd query create tables dividing the Gdelt info, to make other Graphs by theme

CREATE OR REPLACE TABLE
  `myproject-mydataset.gdelt_info_filtering.filtered_spanish_news` AS
SELECT
  -- This select * avoid nulls generated by contrasting with News_in_Spain tables
  *
FROM (
